...
// ═══════════════════════════════════════════════════════
// 🆕 NEW FUNCTION: Claude Reminder Parse
// ═══════════════════════════════════════════════════════

exports.callClaudeReminderParse = onCall(
  {
    secrets: [anthropicApiKey],
    region: 'us-central1',
    timeoutSeconds: 60,
  ...
  async (request) => {
    console.log('⏰ === REMINDER PARSE CALLED ===');
    ...
    const { transcript, timezoneOffsetMinutes } = request.data || {};
    if (!transcript) {
...
Analyze the transcribed text and extract:
- title: Short summary (5-10 words max)
- content: Full transcribed text
- reminderDate: ISO8601 datetime if user mentions time/date, null otherwise

Current UTC time: ${new Date().toISOString()}
User timezone offset: ${timezoneOffsetMinutes} minutes (from UTC)

Examples:
"позвонить маме завтра в 10" → reminderDate: tomorrow at 10:00 local time
"встреча в пятницу в 3 часа дня" → reminderDate: next Friday at 15:00
"купить молоко" → reminderDate: null (no time mentioned)
"deadline конец месяца" → reminderDate: last day of current month at 23:59

Return JSON:
{
  "title": "...",
  "content": "...",
  "reminderDate": "2026-02-19T10:00:00+02:00" or null
}
`;
    ...
    for (const model of modelFallbacks) {
      try {
        ...
        if (!response.ok) {
          const errorText = await response.text();
          lastErrorText = errorText;
          console.error(`❌ Claude Reminder API error (${model}):`, errorText);
    ...
    try {
      ...
      console.error('❌ Failed to parse reminder JSON:', textContent);
      return {
        title: transcript,
        content: transcript,
        reminderDate: null
...
// ═══════════════════════════════════════════════════════
// 🆕 NEW FUNCTION: Whisper + Claude Reminder (single call)
// ═══════════════════════════════════════════════════════

exports.callWhisperAndParseReminder = onCall(
  {
  ...
  async (request) => {
    console.log('⏰ === WHISPER + REMINDER PARSE CALLED ===');
    ...
    const timezoneOffsetMinutes = request.data.timezoneOffsetMinutes ?? 0;
...
`;
      ...
      if (!claudeResponse.ok) {
        ...
        console.error('❌ Claude Reminder API error:', errorText);
        throw new HttpsError('internal', `Claude API error: ${claudeResponse.statusText}`);
      }
      ...
      try {
        ...
        console.error('❌ Failed to parse reminder JSON:', responseText);
      }
      ...
      const remaining = Math.max(0, voiceActionsLimit - (voiceActionsUsed + 1));
