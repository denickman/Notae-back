...
exports.callClaudeProxy = onCall(
                                 ...
                                 async (request) => {
                                     ...
                                     try {
                                         ...
                                         // ─── твои отладочные строки ────────────────────────────────
                                        console.log('🤖 MODEL TO USE:', CLAUDE_TEXT_MODEL);
                                         console.log('🔍 About to call API with body:', {
...
exports.callClaudeVision = onCall(
                                  ...
                                  async (request) => {
                                      ...
                                      try {
                                          ...
                                          const callVisionApi = async (modelName) => {
                                              const apiStartTime = Date.now();
                                              const response = await fetch('https://api.anthropic.com/v1/messages', {
                                                  method: 'POST',
                                                  headers: {
                                              ...
                                              try {
                                                  ...
                                                  throw new HttpsError('internal', 'Claude Vision returned invalid JSON');
                                              }
                                          ...
                                          let apiResult = await callVisionApi(CLAUDE_VISION_MODEL);
                                          
                                          if (!apiResult.response.ok) {
                                              const errorText = apiResult.rawText || '';
                                              const isNotFound = apiResult.response.status === 404 ||
                                              errorText.includes('not_found_error') ||
                                              errorText.includes('model:');
                                              
                                              if (isNotFound && CLAUDE_VISION_FALLBACK_MODEL !== CLAUDE_VISION_MODEL) {
                                                  console.warn('⚠️ Vision model not found, retrying with fallback', {
                                                      primary: CLAUDE_VISION_MODEL,
                                                      fallback: CLAUDE_VISION_FALLBACK_MODEL
                                                  });
                                                  apiResult = await callVisionApi(CLAUDE_VISION_FALLBACK_MODEL);
                                              }
                                          }
                                          
                                          if (!apiResult.response.ok) {
                                              console.error('❌ Claude Vision API error:', apiResult.rawText);
                                              throw new HttpsError('internal', `Claude Vision API error: ${apiResult.response.statusText}`);
                                          }
                                          
                                          const apiDuration = apiResult.apiDuration;
                                          const usedModel = apiResult.modelName;
                                          const result = apiResult.parsed;
